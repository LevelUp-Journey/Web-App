...
export default function ChallengeCard({
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      const deleted = await ChallengeController.deleteChallenge(challenge.id);
      ...
      toast.error(
        dict?.errors?.deleting?.challenge || "Failed to delete challenge",
      );
    } finally {
      setIsDeleting(false);
      ...
      <CardContent className="px-4">
        ...
        <div className="flex items-center justify-between">
          <Button size={"sm"} variant={"ghost"} onClick={handleLike}>
            <Star
              className={cn("text-yellow-400", isLiked && "fill-current")}
              size={14}
            />
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem asChild>
                      ...
                      >
                        {dict?.challenges?.cards?.editChallenge ||
                          "Edit Challenge"}
                      </Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem
                    ...
                    >
                      {dict?.challenges?.cards?.deleteChallenge ||
                        "Delete Challenge"}
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>

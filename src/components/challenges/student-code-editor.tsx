...
export default function StudentCodeEditor({
    ...
    // Efecto para redirigir automáticamente al dashboard después de 20 segundos
    useEffect(() => {
    ...
    }, [showCongratsDialog, router, PATHS.DASHBOARD.ROOT]);
    ...
    useEffect(() => {
        ...
        if (shouldShowGuides && !hasShownGuideDialog && !hasPassedAllTests) {
            setShowGuideDialog(true);
            setHasShownGuideDialog(true);
        }
    }, [shouldShowGuides, hasShownGuideDialog, submitResult]);
                            ...
                            <div className="space-y-4 pr-4">
                                {guides.map((guide) => (
                                    <Button
                                        key={guide.id}
                                        variant="ghost"
                                        className="w-full h-auto p-4 border rounded-lg justify-start text-left hover:bg-muted/50"
                                        onClick={() =>
                        ...
                        <DialogContent>
                            ...
                            <div className="mt-4 space-y-4">
                                <p className="text-sm text-muted-foreground text-center">
                                    {dict?.challenges?.editor?.redirectingIn ||
                                        "Redirecting to dashboard in"}{" "}
                                    <span className="font-bold">
                                        {redirectCountdown}
                                    </span>{" "}
                                    {dict?.challenges?.editor?.seconds ||
                                        "seconds"}
                                    ...
                                </p>
                                <Button
                                    ...
                                    className="w-full"
                                >
                                    {dict?.challenges?.editor?.goToDashboard ||
                                        "Go to Dashboard Now"}
                                </Button>
                            </div>
                        </DialogContent>

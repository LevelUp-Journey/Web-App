...
export function PagesForm({
    ...
    const updatePagesSafely = useCallback((newPages: Page[]) => {
        setPages((prevPages) => {
            ...
            const uniquePages = Array.from(pagesMap.values()).sort(
    ...
    const reloadPagesFromBackend = useCallback(async () => {
        try {
            const response = await GuideController.getGuidePagesByGuideId({
                guideId,
            });
        ...
        try {
            ...
            if ("pages" in response && Array.isArray(response.pages)) {
                ...
                const newPage = response.pages.find(
                    (p) => p.orderNumber === nextOrderNumber,
                );
                if (newPage) {
                ...
                console.error(
                    "Could not extract page data from response:",
                    response,
                );
                        ...
                        <div className="text-center space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">
                                    Select a page from the left or create a new
                                    one
                                </p>
                            </div>
                            <Button

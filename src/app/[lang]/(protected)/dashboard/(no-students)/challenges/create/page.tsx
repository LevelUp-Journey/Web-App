...
export default function CreateChallengePage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      experiencePoints: CHALLENGE_DIFFICULTY_MAX_XP[ChallengeDifficulty.EASY],
  ...
  const onSubmit = form.handleSubmit(async (data: FormData) => {
    ...
    try {
      ...
      const challenge = await ChallengeController.createChallenge(request);

      // Then add guides one by one
      if (selectedGuides.length > 0) {
        const guidePromises = selectedGuides.map((guide) =>
          ChallengeController.addGuideToChallenge(challenge.id, guide.id),
        );
            ...
            <Tabs defaultValue="basic" className="h-full">
                ...
                <form onSubmit={onSubmit} className="flex flex-col gap-4">
                    ...
                    render={({ field, fieldState }) => (
                      <Field data-invalid={fieldState.invalid}>
                        ...
                        <Input
                          ...
                          placeholder={
                            dict?.createChallenge.form.titlePlaceholder
                          }
                          autoComplete="off"
                        />
                        {fieldState.invalid && (
                    ...
                    render={({ field, fieldState }) => (
                      <Field data-invalid={fieldState.invalid}>
                        ...
                        <Input
                          ...
                          aria-invalid={fieldState.invalid}
                          placeholder={
                            dict?.createChallenge.form.tagsPlaceholder
                          }
                          autoComplete="off"
                        />
                        {fieldState.invalid && (
                      ...
                      <Field data-invalid={fieldState.invalid}>
                        ...
                        >
                          ...
                          >
                            <SelectValue
                              placeholder={
                                dict?.createChallenge.form.difficultyPlaceholder
                              }
                            />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value={ChallengeDifficulty.EASY}>
                              {dict?.createChallenge.difficulties.easy}
                            </SelectItem>
                            <SelectItem value={ChallengeDifficulty.MEDIUM}>
                              {dict?.createChallenge.difficulties.medium}
                            </SelectItem>
                            <SelectItem value={ChallengeDifficulty.HARD}>
                              {dict?.createChallenge.difficulties.hard}
                            </SelectItem>
                            <SelectItem value={ChallengeDifficulty.EXPERT}>
                              {dict?.createChallenge.difficulties.expert}
                            </SelectItem>
                          </SelectContent>
                        </Select>
                        {fieldState.invalid && (
                  ...
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <Field data-invalid={fieldState.invalid}>
                        <FieldLabel htmlFor={field.name}>
                          {dict?.createChallenge.form.experiencePoints}
                        </FieldLabel>
                        <FieldDescription>
                          {dict?.createChallenge.form.experiencePointsDescription
                            .replace("{difficulty}", difficulty.toLowerCase())
                            .replace("{maxXP}", maxExperiencePoints.toString())}
                        </FieldDescription>
                        <div className="flex items-center justify-between text-sm text-muted-foreground">
                          <span className="font-semibold text-foreground">
                            {field.value ?? 0} {dict?.createChallenge.form.xp}
                          </span>
                          <span>
              ...
              <TabsContent value="guides" className="space-y-6 mt-6">
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      ...
                      />
                      <FieldDescription>
                        Number of attempts allowed before guides become
                        available.
                      </FieldDescription>
                      {fieldState.invalid && (
                        <FieldError errors={[fieldState.error]} />
                ...
                {selectedGuides.length > 0 && (
                  <div className="space-y-2">
                    <h3 className="text-lg font-semibold">Selected Guides</h3>

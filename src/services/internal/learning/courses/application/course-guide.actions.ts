...
export const addGuideToCourseAction = authenticatedAction(
  ...
  async (data: { guideId: string; courseId: string; orderIndex: number }, { session }) => {
    // TODO: Implement add guide to course logic
    console.log("Adding guide to course with data:", data, "and session:", session);
...
export const removeGuideFromCourseAction = authenticatedAction(
  ...
  async (data: { guideId: string; courseId: string }, { session }) => {
    ...
    console.log("Removing guide from course with data:", data, "and session:", session);
